...
def select_daily_items(ranked: list[RankedItem], *, llm: LLMClient) -> tuple[list[RankedItem], SelectionResult]:
    candidates = ranked[: config.selector_candidates()]
    if not candidates:
    ...
    selected = _apply_selection(ranked, result)
    if not selected:
        ...
        payload.append(
            {
                ...
                "tracks": r.topic_scores.tracks,
                "rank_reason": r.rank_reason,
                "tags": item.tags,
                "abstract_or_excerpt": item.abstract_or_excerpt[:1200],
            }
        )
    return (
...
def _apply_selection(ranked: list[RankedItem], selection: SelectionResult) -> list[RankedItem]:
    ...
    if not picked:
        return []
    pool = list(picked)
    selected_ids = {r.item.item_id for r in pool}
    if not _pool_has_minimums(pool):
        for ranked_item in ranked:
            if ranked_item.item.item_id in selected_ids:
                continue
            pool.append(ranked_item)
            if _pool_has_minimums(pool):
                break
    return score.daily_shortlist(pool)


def _pool_has_minimums(pool: list[RankedItem]) -> bool:
    return len(pool) >= 5 and sum(1 for r in pool if r.item.source_kind == "paper") >= min(config.min_papers(), 5)
