...
def build_graph(
    ...
    g.add_node("planning_brief", node_planning_brief, retry=DEFAULT_RETRY)
    g.add_node("draft_refine", node_draft_refine, retry=DEFAULT_RETRY)
    g.add_node("adversary_review", node_adversary_review, retry=DEFAULT_RETRY)
    g.add_node("evidence_verifier", node_evidence_verifier, retry=DEFAULT_RETRY)
    g.add_node("render_reviewer", node_render_reviewer, retry=DEFAULT_RETRY)
    g.add_node("meta_review", node_meta_review, retry=DEFAULT_RETRY)
    ...
    if config.committee_enabled():
        ...
        g.add_edge("committee", "planning_brief")
    else:
        g.add_node("research", node_research, retry=DEFAULT_RETRY)
        g.add_edge("rank", "research")
        g.add_edge("research", "planning_brief")
    g.add_edge("planning_brief", "draft_refine")
    ...
    g.add_edge("render_reviewer", "meta_review")
    g.add_edge("meta_review", "editor")
